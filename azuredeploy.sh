...
sudo -u $ADMIN_USERNAME sh -c "mkdir /home/$ADMIN_USERNAME/.ssh/;echo Host worker\* > /home/$ADMIN_USERNAME/.ssh/config;echo host 10.0.0.\* >> /home/$ADMIN_USERNAME/.ssh/config; echo StrictHostKeyChecking no >> /home/$ADMIN_USERNAME/.ssh/config;echo UserKnownHostsFile=/dev/null >> /home/$ADMIN_USERNAME/.ssh/config"
...
do
  ...
  sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' | sudo -kS sh -c 'cat /tmp/hosts>>/etc/hosts'"  >> /tmp/azuredeploy.log.$$ 2>&1
  sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' | sudo -kS /tmp/torque-package-mom-linux-x86_64.sh --install"  >> /tmp/azuredeploy.log.$$ 2>&1
  sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' | sudo -kS sh -c 'mkdir /home/$ADMIN_USERNAME/.ssh/;echo Host worker\* > /home/$ADMIN_USERNAME/.ssh/config;echo Host 10.0.0.\* >> /home/$ADMIN_USERNAME/.ssh/config; echo StrictHostKeyChecking no >> /home/$ADMIN_USERNAME/.ssh/config;echo UserKnownHostsFile=/dev/null >> /home/$ADMIN_USERNAME/.ssh/config'"  >> /tmp/azuredeploy.log.$$ 2>&1
  sudo -u $ADMIN_USERNAME scp contrib/init.d/pbs_mom $ADMIN_USERNAME@$worker:/etc/init.d/. >> /tmp/azuredeploy.log.$$ 2>&1
  sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' | sudo -kS sh -c 'chkconfig --add pbs_mom'"  >> /tmp/azuredeploy.log.$$ 2>&1
  sudo -u $ADMIN_USERNAME ssh -tt $worker "echo '$ADMIN_PASSWORD' | sudo -kS sh -c 'service pbs_mom start'"  >> /tmp/azuredeploy.log.$$ 2>&1
